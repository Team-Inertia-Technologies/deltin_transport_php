...
$reasons = "SELECT iReasonID, vName FROM pause_reasons WHERE cStatus='A' ORDER BY iRank ASC";
$result = sql_query($reasons, 'FETCH.PAUSE.REASONS');
$pause_reasons = [];
while ($row = sql_fetch_array($result)) {
    $pause_reasons[] = [
        "id" => intval($row['iReasonID']),
        "lable" => $row['vName']
    ];
}
if (empty($pause_reasons)) {
    http_response_code(200);
    header('Content-Type: application/json');
    echo json_encode([
        "statusCode" => 200,
