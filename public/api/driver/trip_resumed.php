...
$NOW = NOW;
$query = "UPDATE fleet_booking SET cType='R' WHERE iFleet_BookingID='$booking_id' AND iDriverID='$driverID'";
sql_query("INSERT INTO fleet_booking_log (iLogID, iFleet_BookingID, cRefType, vRefName, dtAdded, iUserID, cStatus) VALUES ($log_id, '$booking_id', 'R', 'Trip Resumed', '$NOW', '$driverID', 'A')", 'TRIP.LOG');
...
if (sql_affected_rows() > 0) {
    ...
    echo json_encode([
        "statusCode" => 400,
        "error" => [
            "message" => "Failed to resume trip. Please check booking ID and driver ID."
        ]
    ]);
}
