...
function LogMasterEdit($id, $flag, $mode, $name='', $desc_str='', $user_id=false)
{
	...
	if(empty($name))
	{
		...
		else if($flag=='HTL') $name = GetXFromYID("select vName from gen_hotel where iHotelID=$id");
		else if($flag=='RTY') $name = GetXFromYID("select vName from gen_roomtype where iRoomTypeID=$id");
		else if($flag=='PRP') $name = GetXFromYID("select vName from property where iPropertyID=$id");
		else if($flag=='TRF')
		{
