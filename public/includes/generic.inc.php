...
function memberProfileProgress($memberData)
{
	$fields = [
		'vFirstName' => 5,
		'vLastname' => 5,
	...
	$completionPercentage = round(($filledWeight / $totalWeight) * 100, 2);
...
{
	// Fetch ESC_IGNORE_OFFSET value
	$queryOffset = "SELECT vValue FROM sys_settings WHERE vCode = 'ESC_IGNORE_OFFSET'";
	$resultOffset = sql_query($queryOffset);
	$rowOffset = sql_fetch_assoc($resultOffset);
	$ignoreOffsetDays = $rowOffset['vValue'];

	// Fetch ESC_LEVEL1_FLIGHT value
	$queryLevel1 = "SELECT vValue FROM sys_settings WHERE vCode = 'ESC_LEVEL1_FLIGHT'";
	$resultLevel1 = sql_query($queryLevel1);
	$rowLevel1 = sql_fetch_assoc($resultLevel1);
	$level1Hours = $rowLevel1['vValue'];

	// Fetch ESC_LEVEL2_FLIGHT value
	$queryLevel2 = "SELECT vValue FROM sys_settings WHERE vCode = 'ESC_LEVEL2_FLIGHT'";
	$resultLevel2 = sql_query($queryLevel2);
	$rowLevel2 = sql_fetch_assoc($resultLevel2);
	$level2Hours = $rowLevel2['vValue'];

	// Total hours for level 2
	$totalHours = $level1Hours + $level2Hours;
	$NOW = NOW;

	$tables = ['member_flightrequest', 'member_hotelrequest', 'member_cabrequest'];
	$pendingCounts = [];

	foreach ($tables as $table) {
		// Count for level 1 interval
		$queryLevel1 = "
            SELECT COUNT(*) AS countLevel1 
            FROM $table 
            WHERE cBookingStatus IN ('D', 'A') 
            AND dtADDED >= '{$NOW}' - INTERVAL {$ignoreOffsetDays} DAY
            AND dtADDED <= '{$NOW}' - INTERVAL {$level1Hours} HOUR
            AND dtADDED > '{$NOW}' - INTERVAL {$totalHours} HOUR";

		$resultLevel1 = sql_query($queryLevel1);
		$rowLevel1 = sql_fetch_assoc($resultLevel1);
		$pendingCounts[$table]['level1'] = $rowLevel1['countLevel1'] ?? 0;

		// Count for level 2 interval
		$queryLevel2 = "
            SELECT COUNT(*) AS countLevel2 
            FROM $table 
            WHERE cBookingStatus IN ('D', 'A') 
            AND dtADDED >= '{$NOW}' - INTERVAL {$ignoreOffsetDays} DAY
            AND dtADDED <= '{$NOW}' - INTERVAL {$totalHours} HOUR";
		...
		$resultLevel2 = sql_query($queryLevel2);
		$rowLevel2 = sql_fetch_assoc($resultLevel2);
		$pendingCounts[$table]['level2'] = $rowLevel2['countLevel2'] ?? 0;
	}
...
function isRequestEscalatedRM($reqID, $table)
{
	$queryMaxHours = "SELECT vValue FROM sys_settings WHERE vCode = 'MAX_RESPOND_HOURS_RM_LEVEL1'";
	$resultMaxHours = sql_query($queryMaxHours);
	$rowMaxHours = sql_fetch_assoc($resultMaxHours);
	$maxHours = (int)($rowMaxHours['vValue'] ?? 4);

	$query = "
		SELECT r.iRequestID, r.dtAdded, r.cBookingStatus
		FROM $table r
		WHERE r.cBookingStatus = 'D' AND r.iRequestID = $reqID
			  AND r.dtADDED <= NOW() - INTERVAL $maxHours HOUR";

	$result = sql_query($query);

	return sql_num_rows($result) > 0 ? 1 : 0;
}
function isRequestEscalatedVIP($reqID, $table)
{
	$queryMaxHours = "SELECT vValue FROM sys_settings WHERE vCode = 'MAX_RESPOND_HOURS_VIPSERVICE_LEVEL2'";
	$resultMaxHours = sql_query($queryMaxHours);
	$rowMaxHours = sql_fetch_assoc($resultMaxHours);
	$maxHours = (int)($rowMaxHours['vValue'] ?? 7);

	$queryMaxHours1 = "SELECT vValue FROM sys_settings WHERE vCode = 'MAX_RESPOND_HOURS_RM_LEVEL1'";
	$resultMaxHours1 = sql_query($queryMaxHours1);
	$rowMaxHours1 = sql_fetch_assoc($resultMaxHours1);
	$maxHours1 = (int)($rowMaxHours1['vValue'] ?? 4);
	$maxHours = $maxHours + $maxHours1;
...
function isRequestEscalatedBookingVIP($reqID, $table)
{
	$queryMaxHours = "SELECT vValue FROM sys_settings WHERE vCode = 'MAX_RESPOND_HOURS_VIP_SERVICES_BOOKING_LEVEL1'";
	$resultMaxHours = sql_query($queryMaxHours);
	$rowMaxHours = sql_fetch_assoc($resultMaxHours);
	$maxHours = (int)($rowMaxHours['vValue'] ?? 4);
	...
	$query = "
		SELECT r.iRequestID, r.dtAdded, r.cBookingStatus
		FROM $table r
		WHERE r.cBookingStatus = 'B' AND r.iRequestID = $reqID
			  AND r.dtADDED <= NOW() - INTERVAL $maxHours HOUR";
...
function isRequestEscalatedHOD($reqID, $table)
{
	$queryMaxHours = "SELECT vValue FROM sys_settings WHERE vCode = 'MAX_RESPOND_HOURS_HOD_LEVEL2'";
	$resultMaxHours = sql_query($queryMaxHours);
	$rowMaxHours = sql_fetch_assoc($resultMaxHours);
	$maxHours = (int)($rowMaxHours['vValue'] ?? 7);

	$queryMaxHours1 = "SELECT vValue FROM sys_settings WHERE vCode = 'MAX_RESPOND_HOURS_VIP_SERVICES_BOOKING_LEVEL1'";
	$resultMaxHours1 = sql_query($queryMaxHours1);
	$rowMaxHours1 = sql_fetch_assoc($resultMaxHours1);
	$maxHours1 = (int)($rowMaxHours1['vValue'] ?? 4);
	$maxHours = $maxHours + $maxHours1;
